...
import FlagRow, { yn } from '@/components/FlagRow'
...
export default function ManualReviewDetailPage() {
  ...
  if (!policy || !statusState) {
    return <div className="text-sm text-slate-500">{loading ? 'Loading…' : 'No data.'}</div>
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stage 1 flags */}
      <section className="rounded-2xl border border-slate-200 p-6 shadow-sm">
        <h2 className="mb-2 text-lg font-semibold text-slate-900">Stage 1 flags</h2>
        <div>
          <FlagRow label="Open Claim" fired={policy.openClaim} />
          <FlagRow label="Premium Unpaid" fired={policy.premiumUnpaid} />
          <FlagRow label="Renewal Type Manual" fired={policy.renewalTypeManual} />
          <FlagRow label="System Listed Company" fired={policy.systemListedCompany} />
        </div>
        <p className="mt-3 text-xs text-slate-500">
          Is Frame (informational only, not a flag): <span className="font-medium text-slate-700">{yn(policy.isFrame)}</span>
        </p>
      </section>

      {/* Stage 2 flags */}
      <section className="rounded-2xl border border-slate-200 p-6 shadow-sm">
        <h2 className="mb-2 text-lg font-semibold text-slate-900">Stage 2 flags</h2>
        <div>
          <FlagRow label="D&B No Match" fired={policy.dnbNoMatch} />
          <FlagRow label="D&B Status Inactive" fired={policy.dnbStatusInactive} figure={policy.dnbOperatingStatusLabel} />
          <FlagRow label="D&B Rating Below A" fired={policy.dnbRatingBelowA} figure={policy.dnbRating} />
          <FlagRow
            label="Latest Profit Negative"
            fired={policy.latestProfitNegative}
            figure={formatCurrency(policy.latestNetIncome, policy.currency)}
          />
          <FlagRow
            label="Assets Moved >25% YoY"
            fired={policy.assetsMovedSignificant}
            figure={policy.assetsChangePercent != null ? `${policy.assetsChangePercent > 0 ? '+' : ''}${policy.assetsChangePercent}%` : undefined}
          />
          <FlagRow label="D&B Listed Company" fired={policy.dnbListedCompany} figure={policy.dnbListedExchange} />
        </div>
      </section>
