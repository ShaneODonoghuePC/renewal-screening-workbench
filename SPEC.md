...
> **Screens/routing/status brought current 2026-09-09, reconstructed from the code and `git log` since 22 July (two commits after this note were not previously reflected here at all).** The single biggest change: **on 2026-07-24 (`9036df2`), Assignment & Management, the Auto-Renew Log, and Closed Items — plus the Manual Review/Navins Renew tab split within Assignment & Management itself — were consolidated into one unified, filterable list, "Renewal Management," at `/`** (§5.3). The month-tab bar became a plain dropdown, and its component was deleted (§5.2). Terminal-status ("closed") items no longer move to a separate screen; they stay in the same table, selectable via a Status-filter "Closed" bucket, with their inline Underwriter Workspace switching to a disabled/read-only mode — that read-only mechanism is deliberate and settled, reviewed live and accepted by Shane on 2026-07-24, not an interim state (§5.3, §5.3a, §6.3). **Two specific consequences of the same consolidation are not settled decisions** — they're unreviewed regressions, flagged as such where they're documented (§5.3, §6.3): closed Manual Review rows lost their table-level path into the Risk Assessment panel (the risk evidence behind a status decision, as distinct from the comment thread/activity log, which both survive), and the app lost a "date closed" figure entirely. The panel this document still calls "Risk Quality & Recommendation" was renamed again, to **"Risk Assessment,"** the same day (`72aa316`) — its full content is not yet brought current in this document; that's pending a later revision (§5.4). Data model, seed/synthesis scripts, and technical architecture (§7–§10) are likewise pending later revisions and may still describe the pre-24-July app in places.

> **Prerequisite — historical, satisfied. Data/scripts/architecture brought current 2026-09-09 (§8, §9, §10).** All four countries' real files were provided and anonymized well before this draft's most recent read of the running system: the live dataset (verified against both `local.db` and production Turso, which hold identical data) has 403 policies across DK/NO/SE/FI, not the single-country 129-row figure this blockquote originally gated on. That total reflects a second, later data-generation pass (§8.1, added 2026-07-24) on top of the original anonymization — one this document did not cover until now, and which was applied directly to the live databases rather than to the committed `/data/seed/` files (§8.1 explains the consequence of that). The original prerequisite decision itself (real files only, no synthesizing missing countries, confirmed 2026-07-13) is kept below for provenance, not because it's still gating anything.

> **Original prerequisite text, superseded above:** real screening files for all four countries (DK, NO, SE, FI) must be present in `REAL_DATA_DIR` before running the anonymization script. As of this draft, only the Denmark file has been provided. This was a deliberate decision (2026-07-13) over the alternatives of building DK-only or synthesizing the missing countries — confirmed with Shane specifically to avoid fabricating country data with no real basis.
...
**Consequence, found while verifying this section (2026-09-09): `/data/seed/` (`all.json`, `dk.json`, `no.json`, `se.json`, `fi.json`, `init.sql`) was never regenerated and still holds exactly the original 337-row dataset** — `git log` shows none of these files touched since the initial commit. `scripts/seed-from-json.ts` and `scripts/apply-migration.ts` (§10) both read directly from these files. **The practical effect: re-seeding from the committed repo today reproduces the pre-2026-07-24 337-row state, not what is actually running in `local.db` or production** — the 66 synthesized policies, the VAT unification, and the routing corrections exist only as direct database writes and were never round-tripped back into the committed seed data. This section's original claim that "`/data/seed/` … is what the deployed Vercel build uses" was already a simplification even before 24 July (the running app reads Turso directly at request time, §9 — the JSON files are only ever read once, by the seed-loading script) but is now materially inaccurate as a description of the *data's shape*: the repo alone can no longer reproduce production. This is stated here as a fact about the current build, not a recommendation to regenerate the seed files — that's a decision for Shane, not something to fix silently in a documentation pass.
