...
- **Closed rows** (Manual Review or Navins Renew that has reached one of its terminal statuses, §6.3): stay in this same table — selectable via the Status filter's "Closed" bucket — rather than moving to a separate screen. The expand caret reveals the same Underwriter Workspace component (§5.4b) any open row gets, but in a `readOnly` mode: the Status and Assigned-to `<select>`s are disabled (still showing the real current value), the comment-entry box is hidden entirely, and the mutation handlers themselves short-circuit on the `readOnly` flag as defense in depth, independent of the disabled attribute. **This is deliberate, settled product behaviour** — reviewed live and accepted 2026-07-24, not an interim state pending a proper Closed Items screen. The Actions column shows nothing for a closed row (no Review button, no Assign-to-me, no assigned-to select) — for a closed Manual Review policy this means the table itself no longer offers a way into the Risk Assessment panel; the panel is still reachable directly via `/review/manual/[id]` (§5.4) for anyone with the policy id, since that standalone route has no closed-state gating of its own. There is no longer a "date closed" figure surfaced anywhere in the UI — the activity log inside the expanded (read-only) workspace still shows timestamped status-change history, but nothing computes or displays a single "date closed" value the way the former Closed Items screen did.
