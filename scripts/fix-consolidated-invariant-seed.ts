// Corrects the consolidated-accounts invariant (scripts/lib/fixConsolidatedInvariantPlan.ts)
...
//
// Run with: npx tsx scripts/fix-consolidated-invariant-seed.ts [--apply]
// Without --apply, this is a dry run: it prints exactly what it would do and writes nothing.

import fs from 'fs'
import path from 'path'
import { computeFixConsolidatedInvariantPlanForRows, printFixConsolidatedInvariantPlanReport } from './lib/fixConsolidatedInvariantPlan'
...
async function main() {
  ...
  for (const file of FILES) {
    ...
    const plan = computeFixConsolidatedInvariantPlanForRows(rows)
